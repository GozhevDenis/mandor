...
long long reg_t::volume()         //const
{
   ...
   long long res = 1;
   // directive #if used in original file
   if ( MC_X ) res *= ( max[0] - min[0] + 1 );
   if ( MC_Y ) res *= ( max[1] - min[1] + 1 );
   if ( MC_Z ) res *= ( max[2] - min[2] + 1 );
...
regList_t::slice( int axis, int coord )
{
   ...
   for ( auto it  = list.begin() ; it != end ; ++it ) {
      if ( it->getMin( axis ) < coord
      &&   it->getMax( axis ) > coord ) {
...
regList_t::verify()
{
   if ( !list.empty() ) {              ////?? Why is not it assert? return 0?
      for ( auto it  = list.begin() ; it != list.end() ; ++it ) {
         // Checks that all regions are sorted normally.
         if ( it->volume() <= 0 )
            return 1;
      }
      for ( auto it1 = list.begin() ; it1 != list.end() ; ++it1 )
         for ( auto it2 = it1 + 1 ;
               it2 != list.end() ; ++it2 )                       // Checks that there are no overlapping.
